...
// test command. to be enabled only for debug
    APP_Commands_Init();
    appData.ipMode = 4;
...
void APP_Tasks(void) {
    ...
    /* Bulk of this state machine isa pplicable only with APP_Command_init and is mainly used for debug*/
    switch (appData.state) {
        ...
        case APP_TCPIP_WAIT_FOR_TCPIP_INIT:
        {
            ...
            if (tcpipStat < 0) { // some error occurred
                ...
                appData.state = APP_TCPIP_WAITING_FOR_SERIAL_NUM;

            }
            break;
        }
        case APP_TCPIP_WAITING_FOR_SERIAL_NUM:
            if(0!=strlen(app_controlData.devSerialStr)){
                appData.state = APP_TCPIP_WAITING_FOR_CLOUD_CONFIG;
            }
            break;
        case APP_TCPIP_WAITING_FOR_CLOUD_CONFIG:
